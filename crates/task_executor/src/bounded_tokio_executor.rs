use crate::executor::{ExecutorFuture, TaskExecutor};
...
impl TaskExecutor for BoundedTokioExecutor {
    ...
    /// Note: tokio manages spawn_blocking tasks on a separate threadpool from the IO tasks,
    /// so, subject to benchmarking, this throttling might be unnecessary.
    ///
    /// # Example
    ///
    /// ```
    ...
    /// });
    fn spawn<F, T>(&self, task: F) -> ExecutorFuture<T, Self::SpawnBlockingError>
    where
        F: Future<Output = T> + Send + 'static,
        T: Send + 'static,
    {
        Box::pin(async move { task::spawn(task).await })
    }
}
