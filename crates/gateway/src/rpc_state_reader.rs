...
use crate::rpc_objects::{BlockId, GetNonceParams, RpcResponse, Tag};
...
impl RpcStateReader {
    pub fn new(chain_id: ChainId) -> Self {
        let url = match chain_id.0.as_str() {
            "SN_MAIN" => "https://papyrus-for-mempool-mainnet.sw-dev.io/rpc/v0_6".to_string(),
            "SN_SEPOLIA" => "https://papyrus-for-mempool-sepolia.sw-dev.io/rpc/v0_6".to_string(),
            "SN_INTEGRATION_SEPOLIA" => {
                "https://papyrus-for-mempool-integration-sepolia.sw-dev.io/rpc/v0_6".to_string()
            }
        ...
        if response.status().is_success() {
            ...
            match rpc_response {
                ...
                RpcResponse::Error(rpc_error_response) => match rpc_error_response.error.code {
                    24 => Err(StateError::StateReadError("Block not found".to_string())),
                    20 => Err(StateError::StateReadError(format!(
