...
impl RpcStateReader {
    pub fn new(chain_id: ChainId, spec_version: &str) -> Self {
        let url = match chain_id.0.as_str() {
            "SN_MAIN" => {
                "https://papyrus-for-mempool-mainnet.sw-dev.io/rpc/".to_string() + spec_version
            }
            "SN_SEPOLIA" => {
                "https://papyrus-for-mempool-sepolia.sw-dev.io/rpc/".to_string() + spec_version
            }
            "SN_INTEGRATION_SEPOLIA" => {
                "https://papyrus-for-mempool-integration-sepolia.sw-dev.io/rpc/".to_string()
                    + spec_version
            }
