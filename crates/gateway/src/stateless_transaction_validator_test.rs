...
use crate::starknet_api_test_utils::{
    ...
    non_zero_resource_bounds_mapping, zero_resource_bounds_mapping, NON_EMPTY_RSOURCE_BOUNDS,
};
...
#[case::zero_l2_gas_resource_bounds(
    ...
    create_external_invoke_tx_for_testing(
        create_resource_bounds_mapping(NON_EMPTY_RSOURCE_BOUNDS, ResourceBounds::default())
    ),
    Err(TransactionValidatorError::ZeroFee{
...
#[case::valid_l2_gas_invoke_tx(
    ...
    create_external_invoke_tx_for_testing(
        create_resource_bounds_mapping(ResourceBounds::default(), NON_EMPTY_RSOURCE_BOUNDS)
    ),
    Ok(())
)]
// General flow.
