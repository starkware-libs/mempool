...
async fn test_add_transaction_declare() {
    ...
    let serialized =
        serde_json::to_string(&external_tx).expect("Failed to serialize ExternalTransaction");

    let valid_request = Request::post("/add_transaction")
        .body(Body::from(serialized))
        .unwrap();
    let valid_response = Gateway::handle_request(valid_request).await.unwrap();
    ...
    let invalid_response = Gateway::handle_request(invalid_request).await;
