...
async fn test_add_transaction(#[case] json_file_path: &Path, #[case] expected_response: &str) {
    ...
    let network = GatewayNetworkComponent::new(tx_gateway_2_mempool, rx_mempool_2_gateway);

    let mut app_state = AppState {
        gateway_state: gateway_state.clone(),
        network: Arc::new(network),
    };
    let response = add_transaction(State(app_state.clone()), tx.clone().into())
        .await
        .into_response();
    ...
    let response = add_transaction(State(app_state), tx.into())
        .await
        .into_response();
