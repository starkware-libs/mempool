...
async fn test_add_transaction(#[case] json_file_path: &Path, #[case] expected_response: &str) {
    ...
    let response =
        add_transaction(State(gateway_state.clone()), tx.clone().into()).await.into_response();
    ...
    let response = add_transaction(State(gateway_state), tx.into()).await.into_response();
