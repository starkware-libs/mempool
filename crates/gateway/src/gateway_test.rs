...
use crate::starknet_api_test_utils::{test_invoke_tx_params, TestTxParams};
...
#[tokio::test]
#[rstest]
#[case::valid_invoke_tx(test_invoke_tx_params(false))]
#[case::valid_deploy_account_tx(crate::starknet_api_test_utils::test_deploy_account_tx_params())]
async fn test_add_tx(#[case] test_tx_params: TestTxParams) {
    // TODO: Add fixture.
    ...
    let app_state = app_state(mempool_client, test_tx_params.state_reader_factory);

    let tx = test_tx_params.external_tx;
...
fn calculate_hash(external_tx: &ExternalTransaction) -> TransactionHash {
    if matches!(external_tx, ExternalTransaction::Declare(_)) {
