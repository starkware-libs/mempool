...
use crate::rpc_state_reader::RpcReader;
...
impl StatefulTransactionValidator {
    pub fn validate(
        ...
        let latest_block_info = self.get_latest_block_info()?;
        let state_reader = RpcReader::new(
            self.config.rpc_url.clone(),
            self.config.json_rpc_version.clone(),
            latest_block_info.block_number,
        );
        self.run_validate(
            state_reader,
            latest_block_info,
    ...
    fn run_validate<S: BlockifierStateReader>(
        &self,
        state_reader: S,
    ...
    pub fn get_latest_block_info(&self) -> StatefulTransactionValidatorResult<BlockInfo> {
        let rpc_reader = RpcReader::new_latest(
            self.config.rpc_url.clone(),
            self.config.json_rpc_version.clone(),
        );
