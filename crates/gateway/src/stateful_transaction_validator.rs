...
impl StatefulTransactionValidator {
    pub fn run_validate(
        ...
        let latest_block_number = latest_block_info.block_number;
        let state_reader = state_reader_factory.get_state_reader(latest_block_number);
...
fn skip_stateful_validations(
    ...
    match tx {
        RPCTransaction::Invoke(RPCInvokeTransaction::V3(tx)) => {
            ...
            // deploy_account, and the account nonce is zero, meaning the account was not deployed
            // yet. The mempool also verifies that the deploy_account transaction exists.
            Ok(tx.nonce >= Nonce(Felt::ONE)
