...
use crate::rpc_state_reader::{MempoolStateReader, StateReaderFactory};
...
pub struct StatefulTransactionValidator<T, S>
where
    T: StateReaderFactory<S>,
    S: MempoolStateReader,
{
    pub config: StatefulTransactionValidatorConfig<T, S>,
    _marker: PhantomData<S>,
}

impl<T, S> StatefulTransactionValidator<T, S>
where
    T: StateReaderFactory<S>,
    S: MempoolStateReader,
{
    pub fn run_validate(
        ...
        let latest_block_info = get_latest_block_info(&self.config.state_reader_factory)?;
        let state_reader =
            self.config.state_reader_factory.get_state_reader(latest_block_info.block_number);
...
pub fn get_latest_block_info<S: MempoolStateReader>(
    ...
    let state_reader = state_reader_factory.get_latest_state_reader();
    Ok(state_reader.get_block_info()?)
}

pub struct StatefulTransactionValidatorConfig<T, S>
where
    T: StateReaderFactory<S>,
    S: MempoolStateReader,
{
