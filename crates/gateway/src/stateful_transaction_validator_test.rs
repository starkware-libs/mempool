...
fn test_stateful_transaction_validator(
    ...
    // TODO(yael 17/4/24): replace with the new test util the Arni is adding to the blockifier
    // that will create a test_state_reader.
    ...
    let state_reader = DictStateReader {
        address_to_class_hash: state.state.address_to_class_hash.clone(),
        class_hash_to_class: state.state.class_hash_to_class.clone(),
        storage_view: state.state.storage_view.clone(),
        ..Default::default()
    };

    let stateful_validator_config = StatefulTransactionValidatorConfig {
    ...
    let result = StatefulTransactionValidator::create_and_run_validate(
        &stateful_validator_config,
        state_reader,
