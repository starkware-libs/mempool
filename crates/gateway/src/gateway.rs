...
fn process_tx(
    ...
    let optional_class_info = match &tx {
        ExternalTransaction::Declare(declare_tx) => Some(compile_contract_class(declare_tx)?),
        _ => None,
    };
...
fn compile_contract_class(declare_tx: &ExternalDeclareTransaction) -> GatewayResult<ClassInfo> {
