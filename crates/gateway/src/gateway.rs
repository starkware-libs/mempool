...
impl Gateway {
    ...
    pub async fn create_for_testing(network_component: GatewayNetworkComponent) -> Gateway {
        let stateless_tx_validator_config =
        ...
        let gateway_config = GatewayConfig {
            network_config,
            stateless_transaction_validator_config: stateless_tx_validator_config,
            stateful_transaction_validator_config,
        };

        let state_reader_factory =
            Arc::new(crate::state_reader_test_utils::test_state_reader_factory());

        Gateway::new(gateway_config, network_component, state_reader_factory)
    }
}
