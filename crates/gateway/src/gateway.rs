...
use starknet_mempool_types::mempool_types::{Account, MempoolClientInterface, MempoolInput};
...
pub struct AppState {
    ...
    pub mempool: Arc<Box<dyn MempoolClientInterface>>,
}
...
impl Gateway {
    pub fn new(
        ...
        mempool: Box<dyn MempoolClientInterface>,
    ) -> Self {
...
async fn add_tx(
    ...
    // TODO(Elin/Dafna, 1/5/2024): Replace "expect" with required error handling.
    app_state
        .mempool
        .add_tx(mempool_input)
        .await
        .expect("Communication should succeed")
