...
impl Gateway {
    ...
    pub async fn build_server(&self) -> Result<(), GatewayError> {
        // pub async fn build_server(self: Arc<Self>) -> Result<(), GatewayError> {
        ...
        let make_service = make_service_fn(move |_conn| {
            ...
            async move {
                ...
                Ok::<_, Infallible>(service_fn(move |req| {
                    // Now it's safe to move self_clone, as it's an Arc clone.
                    ...
                    async move {
                        // Use the clone of `ctx` inside the async block.
                        self_clone_inner.handle_request(ctx_clone, req).await
                    }
                }))
            }
        });
