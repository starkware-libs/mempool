...
pub fn initialize_papyrus_test_state(
    ...
    contract_instances.iter().for_each(|(contract, n_instances)| {
        for instance in 0..*n_instances {
            // Declare and deploy the contracts
            declared_classes.insert(contract.get_class_hash(), Default::default());
            deployed_contracts.insert(contract.get_instance_address(instance), contract.get_class_hash());
            // Fund the accounts.
            match contract {
...
pub async fn run_papyrus_rpc_server(storage_reader: StorageReader) -> SocketAddr {
    let gateway_config = RpcConfig::default();
    let highest_block = get_test_highest_block();
