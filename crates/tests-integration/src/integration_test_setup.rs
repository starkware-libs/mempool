...
pub struct IntegrationTestSetup {
    ...
    pub rx_mempool_to_batcher: Receiver<Vec<ThinTransaction>>,
...
impl IntegrationTestSetup {
    ...
    pub async fn add_tx(&self, tx: &ExternalTransaction) {
        self.gateway_client.assert_add_tx_success(tx).await;
    }

    pub async fn add_tx_error(&self, tx: &ExternalTransaction) -> GatewayError {
    ...
    pub async fn get_txs(&mut self, n_txs: usize) -> Vec<ThinTransaction> {
        ...
        self.rx_mempool_to_batcher.recv().await.unwrap()
    }
}
