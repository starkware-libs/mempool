...
async fn test_mempool_emptying_scenarios() {
    let n_accounts = 2;
    let (mut mock_running_system, mut tx_generator) = setup_with_tx_generation(n_accounts).await;

    // asking for more txs than the number stored should return all the stored txs.
    let contract0_tip1 = tx_generator
        .account_with_id(0)
        .generate_valid_with_overrides(invoke_tx_args! { tip: Tip(1)});
    ...
    let contract0_tip3 = tx_generator
        .account_with_id(0)
        .generate_valid_with_overrides(invoke_tx_args! { tip: Tip(3) });
    let contract1_tip2 = tx_generator
        .account_with_id(1)
        .generate_valid_with_overrides(invoke_tx_args! { tip: Tip(2) });
