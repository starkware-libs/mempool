...
async fn test_end_to_end() {
    ...
    // Only the nonce-0 txs should be returned from get_txs, because we haven't merged
    // queue-replenishment yet.
    let mut expected_tx_hashes_from_get_txs =
    ...
    expected_tx_hashes_from_get_txs.sort();
