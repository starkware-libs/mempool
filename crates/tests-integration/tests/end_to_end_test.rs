...
async fn test_mempool_emptying_scenarios() {
    ...
    let (mut mock_running_system, mut tx_generator) =
        IntegrationTestSetup::new_with_tx_generator(n_accounts).await;

    // asking for more txs than the number stored should return all the stored txs.
    let contract0_tip1 =
        tx_generator.account(0).generate_valid_with_overrides(invoke_tx_args! { tip: Tip(1)});
    ...
    let contract0_tip3 =
        tx_generator.account(0).generate_valid_with_overrides(invoke_tx_args! { tip: Tip(3) });
    let contract1_tip2 =
        tx_generator.account(1).generate_valid_with_overrides(invoke_tx_args! { tip: Tip(2) });
