...
async fn test_mempool_emptying_scenarios() {
    ...
    let contract0_tip1 = tx_generator.generate(0, invoke_tx_args! { tip: Tip(1)});
    dbg!(&mock_running_system);
    ...
    let contract0_tip3 = tx_generator.generate(0, invoke_tx_args! { tip: Tip(3) });
    let contract1_tip2 = tx_generator.generate(1, invoke_tx_args! { tip: Tip(2) });
