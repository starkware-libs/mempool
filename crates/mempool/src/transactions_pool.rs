...
pub struct TransactionsPool {
    ...
    pool: HashMap<TransactionHash, ThinTransaction>,
...
impl TransactionsPool {
    pub fn push(&mut self, tx: ThinTransaction) -> MempoolResult<()> {
        ...
        match self.pool.entry(tx_hash) {
            hash_map::Entry::Vacant(txs) => {
                txs.insert(tx.clone());
            }
    ...
    pub fn remove(&mut self, tx_hash: &TransactionHash) -> MempoolResult<ThinTransaction> {
        ...
        match txs_from_account_entry {
            hash_map::Entry::Occupied(mut entry) => {
                ...
                assert!(
                    txs_from_account.remove(&tx.nonce).is_some(),
