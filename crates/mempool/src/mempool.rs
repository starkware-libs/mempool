...
use crate::priority_queue::{self, TransactionPriorityQueue};
...
impl Mempool {
    pub fn new(
        ...
        let valid_txs: Vec<ThinTransaction> = inputs
            .into_iter()
            .map(|input| {
                // Attempts to insert a key-value pair into the mempool's state. Returns `None` if
                // the key was not present, otherwise returns the old value while
        ...
        mempool.txs_queue = priority_queue::TransactionPriorityQueue::from(valid_txs);
