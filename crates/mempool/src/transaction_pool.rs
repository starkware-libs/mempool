...
impl TransactionPool {
    ...
    pub fn remove_txs_up_to_nonce(&mut self, address: ContractAddress, nonce: Nonce) {
        let removed_txs = self.txs_by_account.remove_txs_up_to_nonce(address, nonce);
    ...
    fn remove_txs_up_to_nonce(
        ...
        match self.0.get_mut(&address) {
            Some(btree_map) => {
                ...
                let nonces_to_remove: Vec<Nonce> =
                    btree_map.range(..=nonce).map(|(&n, _)| n).collect();
