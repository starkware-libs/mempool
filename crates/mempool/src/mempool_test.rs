...
fn test_commit_block() {
    ...
    mempool.commit_block(0, &[TransactionHash(StarkFelt::TWO)], HashMap::default()).unwrap();

    let _txs = mempool.get_txs(1).unwrap();
    mempool.commit_block(0, &[TransactionHash(StarkFelt::ONE)], HashMap::default()).unwrap();
}
