...
fn test_get_txs(#[case] requested_txs: usize) {
    ...
    let ref_txs_iterator = add_tx_input.iter().map(|input| TransactionReference::new(&input.tx));
    let txs_iterator = add_tx_input.iter().map(|input| input.tx.clone());

    let mut mempool: Mempool = MempoolState::new(txs_iterator, ref_txs_iterator).into();

    let txs = mempool.get_txs(requested_txs).unwrap();

    let sorted_txs = vec![
        add_tx_input[1].tx.clone(), // tip 100
        add_tx_input[0].tx.clone(), // tip 50
...
fn test_adding_sequential_nonce_txs(mut mempool: Mempool) {
    ...
    let queue_txs = [TransactionReference::new(&input_nonce_0.tx)];
    let pool_txs = [input_nonce_0.tx, input_nonce_1.tx];
