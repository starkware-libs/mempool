...
macro_rules! add_tx_input {
    ...
    // Pattern for tx_nonce and account_nonce.
    (tx_nonce: $tx_nonce:expr, account_nonce: $account_nonce:expr) => {
        add_tx_input!(tip: 1, tx_hash: Felt::ZERO, sender_address: "0x0", tx_nonce: $tx_nonce, account_nonce: $account_nonce)
    };
}
...
fn test_add_tx_sequential_nonces(mut mempool: Mempool) {
    // Setup: add two transactions of the same account with sequential nonces.
    let input_nonce_0 = add_tx_input!(tx_nonce: 0_u8, account_nonce: 0_u8);
    let input_nonce_1 = add_tx_input!(tx_nonce: 1_u8, account_nonce: 0_u8);
    ...
    // Ensure that the first transaction is in the queue and both transactions are in the pool.
