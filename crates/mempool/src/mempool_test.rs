...
fn test_commit_block_includes_all_transactions() {
    ...
    let tx_address1_nonce3 = add_tx_input!(tip: 1, tx_hash: 3, sender_address: "0x1", tx_nonce: 3_u8, account_nonce: 3_u8).tx;

    let queued_txs = vec![tx_address0_nonce4.clone(), tx_address1_nonce3.clone()];
    let pool_txs = vec![tx_address0_nonce4, tx_address0_nonce5.clone(), tx_address1_nonce3];
    ...
    assert_eq_mempool_queue(&mempool, &[tx_address0_nonce5])
}
