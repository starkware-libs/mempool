...
fn test_get_txs(#[case] requested_txs: usize) {
    ...
    mempool.add_tx(tx1.clone(), &account_state1).unwrap();
    mempool.add_tx(tx2.clone(), &account_state2).unwrap();
    mempool.add_tx(tx3.clone(), &account_state3).unwrap();
...
fn test_add_tx() {
    ...
    assert_eq!(mempool.priority_queue.pop_last().unwrap(), Transaction(tx2));
    assert_eq!(mempool.priority_queue.pop_last().unwrap(), Transaction(tx1));
}
