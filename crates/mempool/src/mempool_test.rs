...
// transactions.
#[track_caller]
fn verify_mempool_queue_eq(mempool: &Mempool, expected_queue: &[ThinTransaction]) {
...
fn test_get_txs(#[case] requested_txs: usize) {
    ...
    verify_mempool_queue_eq(&mempool, remaining_txs);
}
...
fn test_add_tx_with_duplicate_tx(mut mempool: Mempool) {
    ...
    verify_mempool_queue_eq(&mempool, &[same_input.tx])
}
...
fn test_add_tx_with_identical_tip_succeeds(mut mempool: Mempool) {
    ...
    verify_mempool_queue_eq(&mempool, &[input1.tx, input2.tx]);
}
...
fn test_tip_priority_over_tx_hash(mut mempool: Mempool) {
    ...
    verify_mempool_queue_eq(&mempool, &[input_big_tip_small_hash.tx, input_small_tip_big_hash.tx])
}
