...
fn check_mempool_state_eq(
    ...
    assert_eq!(mempool.tx_pool().tx_pool(), &expected_txs);
}

// Asserts that the transactions in the mempool are in ascending order as per the expected
// transactions.
#[track_caller]
fn check_mempool_queue_eq(mempool: &Mempool, expected_queue: &[ThinTransaction]) {
    ...
    for (i, (expected_tx, mempool_tx)) in enumerate(zip_eq(expected_queue, mempool_txs)) {
        assert_eq!(expected_tx, *mempool_tx, "{i}'th tx in the queue doesn't equal the expected");
    }
}
...
fn test_get_txs(#[case] requested_txs: usize) {
    ...
    check_mempool_queue_eq(&mempool, remaining_txs);
}
...
fn test_add_tx_multi_nonce_success(mut mempool: Mempool) {
    ...
    check_mempool_state_eq(&mempool, expected_pool_all_txs, expected_queue_only_zero_nonce_txs);
}
...
fn test_add_tx_with_duplicate_tx(mut mempool: Mempool) {
    ...
    check_mempool_queue_eq(&mempool, &[same_input.tx])
}
...
fn test_add_tx_with_identical_tip_succeeds(mut mempool: Mempool) {
    ...
    check_mempool_queue_eq(&mempool, &[input1.tx, input2.tx]);
}
...
fn test_tip_priority_over_tx_hash(mut mempool: Mempool) {
    ...
    check_mempool_queue_eq(&mempool, &[input_big_tip_small_hash.tx, input_small_tip_big_hash.tx])
}
