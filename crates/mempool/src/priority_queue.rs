...
pub struct TransactionQueue(BTreeSet<PrioritizedTransaction>);
...
impl From<Vec<TransactionReference>> for TransactionQueue {
    fn from(transactions: Vec<TransactionReference>) -> Self {
        TransactionQueue(BTreeSet::from_iter(transactions.into_iter().map(PrioritizedTransaction)))
    }
}

#[derive(Clone, Debug, derive_more::Deref, derive_more::From)]
struct PrioritizedTransaction(pub TransactionReference);
