...
use crate::config::{MemPoolNodeConfig, ModuleExecutionConfig, ModulesConfig};
...
fn test_valid_config() {
    ...
    let expected_config = MemPoolNodeConfig {
        modules: ModulesConfig {
            gateway_module: ModuleExecutionConfig { execute: true },
            mempool_module: ModuleExecutionConfig { execute: false },
        },
...
fn test_modules_config() {
    ...
    // Invalidate the gateway  module and check that the validator finds an error.
    config.modules.gateway_module.execute = false;

    assert_matches!(config.validate(), Err(e) => {
        let parse_err = ParsedValidationErrors::from(e);
        let mut error_msg = String::new();
        for error in parse_err.0 {
            if error.param_path == "modules.__all__" {
                error_msg.push_str(&error.code);
                break;
            }
        }
        assert_eq!(error_msg, "Invalid modules configuration.");
    });

    // Validate the mempool module and check that the validator finds no errors.
    config.modules.mempool_module.execute = true;
    assert!(config.validate().is_ok());
}
