...
impl ComponentATrait for ComponentAClientHttp {
    async fn a_get_value(&self) -> ValueA {
        ...
        let req = Request::post(self.uri.clone())
            .header("Content-Type", "application/octet-stream")
            .body(Body::from(
                bincode::serialize(&a_get_value).expect("Request serialization should succeed"),
            ))
...
impl ComponentBTrait for ComponentBClientHttp {
    async fn b_get_value(&self) -> ValueB {
        ...
        let req = Request::post(self.uri.clone())
            .header("Content-Type", "application/octet-stream")
            .body(Body::from(
                bincode::serialize(&b_get_value).expect("Request serialization should succeed"),
            ))
            .expect("Request builidng should succeed");
