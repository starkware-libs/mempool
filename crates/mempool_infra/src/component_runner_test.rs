...
use test_component1::{TestComponent1, TestConfig1};

#[tokio::test]
async fn test_testruner1() {
    let test_config = TestConfig1 { config1: true };
    assert_matches!(TestComponent1::start(test_config.clone()).await, Ok(()));
}

use test_component2::{TestComponent2, TestConfig2};

#[tokio::test]
async fn test_testruner2() {
    let test_config = TestConfig2 { config2: 42 };
    assert_matches!(TestComponent2::start(test_config).await, Ok(()));
