...
use crate::component_client::DummyClient;
...
mod test_component_a {
    ...
    impl ComponentCreator<TestConfigA, (), ()> for TestComponentA {
        fn create(config: TestConfigA, client: Option<Arc<DummyClient>>) -> Self {
...
mod test_component_b {
    ...
    impl ComponentCreator<TestConfigB, (), ()> for TestComponentB {
        fn create(config: TestConfigB, client: Option<Arc<DummyClient>>) -> Self {
            assert!(client.is_none());
            Self { config }
        }
    }
...
async fn test_component_a() {
    ...
    let mut component = TestComponentA::create(test_config, None);
...
async fn test_component_b() {
    ...
    let mut component = TestComponentB::create(test_config, None);
    assert_matches!(component.start().await, Ok(()));
