...
use crate::component_definitions::{
    ...
    DESERIALIZAION_FAILED_REASON, X_INTERNAL_ERROR_REASON_HEADER,
};
    ...
    async fn handler(
        ...
        let http_response = match deserialize(&body_bytes) {
            Ok(component_request) => {
                ...
                HyperResponse::builder().header(CONTENT_TYPE, APPLICATION_OCTET_STREAM).body(
                    Body::from(
            ...
            Err(error) => HyperResponse::builder()
                .status(StatusCode::BAD_REQUEST)
                .header(X_INTERNAL_ERROR_REASON_HEADER, DESERIALIZAION_FAILED_REASON)
