...
async fn test_testruner1() {
    ...
    let test_component = TestComponent1::new();
    assert_matches!(
        test_component
...
async fn test_testruner2() {
    ...
    let test_component = TestComponent2::new();
    assert_matches!(
        test_component
            .start_component(Some(Box::new(&test_config)))
            .await,
    ...
    assert_matches!(test_component.start_component(Some(Box::new(&test_config))).await, Err(e) => {
...
async fn test_run_from_vector() {
    ...
    // let tc3 = TestComponent3::new();
    let components: Vec<Box<dyn ComponentRunner>> = vec![
        Box::new(TestComponent1::new()),
        Box::new(TestComponent2::new()),
        Box::new(TestComponent2::new()),
        Box::new(TestComponent3::new()),
    ];

    let configs: Vec<Option<Box<&(dyn ExtendConfig + Send + Sync)>>> = vec![
        Some(Box::new(&common_config.test_config_1)),
        Some(Box::new(&common_config.test_config_1)),
        Some(Box::new(&common_config.test_config_2)),
        Some(Box::new(&common_config.test_config_3)),
    ];
    ...
    for (i, (component, config)) in components.into_iter().zip(configs.into_iter()).enumerate() {
        let ret_val = component.start_component(config).await;
